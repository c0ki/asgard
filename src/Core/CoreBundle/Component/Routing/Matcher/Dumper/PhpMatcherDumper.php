...
EOF;
    ...
    private function compileRoute(Route $route, $name, $supportsRedirections, $parentPrefix = null)
    {
        ...
        if ($req = $route->getRequirement('_site')) {
            $sites = explode('|', strtolower($req));
        }
...
EOF;
        ...
        if ($sites) {
            if (1 === count($sites)) {
                $code .= <<<EOF
            if (\$this->request->attributes->get('_site') != '$sites[0]') {
                \$allow[] = '$sites[0]';
                goto $gotoname;
            }
...
EOF;
                ...
                $sites = implode("', '", $sites);
                $code .= <<<EOF
            if (!in_array(\$this->request->attributes->get('_site'), array('$sites'))) {
                \$allow = array_merge(\$allow, array('$sites'));
                goto $gotoname;
            }
