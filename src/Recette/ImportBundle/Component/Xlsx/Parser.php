...
namespace Recette\ImportBundle\Component\Xlsx;
...
class Parser
{
    ...
    const TemporaryPath = 'recette/xlsxparser/tmp/';
    ...
    public function parse($inputFile, $headLine = 0, $limitRow = null)
    {
        ...
        $zip = new \ZipArchive();
        $zip->open($inputFile->getPathname());
        $zip->extractTo($temporaryDir);
        $zip->close();

        $xmlValuesTemporaryFilename =
            $temporaryDir . DIRECTORY_SEPARATOR . 'xl' . DIRECTORY_SEPARATOR . "sharedStrings.xml";
        $xmlValues = new \SimpleXMLElement($xmlValuesTemporaryFilename, null, true);
        $xmlValues->registerXPathNamespace('default', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main');

        $xmlSheetTemporaryFilename =
            $temporaryDir . DIRECTORY_SEPARATOR . 'xl' . DIRECTORY_SEPARATOR . 'worksheets' . DIRECTORY_SEPARATOR
            . "sheet1.xml";
        $xmlSheet = new \SimpleXMLElement($xmlSheetTemporaryFilename, null, true);
        $xmlSheet->registerXPathNamespace('default', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main');
